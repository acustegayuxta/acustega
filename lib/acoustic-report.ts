...
export const REPORT_SYSTEM_PROMPT = `You are an expert acoustic engineer. Your sole task is to analyze the input and output a single valid JSON object.
...
- If the input lacks sufficient acoustic detail, still return the JSON using reasonable generic values based on the space name — never respond with plain text.
...
export function buildPDF(data: ReportData, spaceLabel: string): ArrayBuffer {
...
export function parseReportJson(raw: string): ReportData {
...
export async function buildCasosContext(): Promise<string> {
  try {
