...
function IsometricRoom() {
  return (
    <svg viewBox="0 0 360 240" className="w-full h-full" fill="none" aria-hidden>
      ...
      {/* ── Acoustic panels — left wall (cyan) ── */}
      ...
      {/* ── Acoustic panels — right wall (amber) ── */}
  ...
  return (
    ...
    >
      ...
      <Link href="/" className="flex items-center gap-2.5 select-none">
        ...
        <span
          className="text-sm font-bold tracking-[0.2em] leading-none"
          style={{ color: CREAM }}
        >
      ...
      <nav className="flex items-center gap-6">
        ...
        <Link
          href="/asesor"
          className="px-4 py-1.5 rounded-lg text-[12px] font-semibold tracking-wide transition-opacity hover:opacity-85"
  ...
  return (
    <div style={{ backgroundColor: BG, fontFamily: "var(--font-outfit)", color: CREAM, minHeight: "100vh" }}>
      ...
      <section className="min-h-screen flex items-center pt-16 px-6">
        <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center py-20">
          ...
          <div className="flex flex-col gap-7">
            ...
            {/* Badge */}
            <span
              className="self-start inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-semibold tracking-[0.15em] uppercase"
            ...
            >
              Tu espacio sin{" "}
              <span style={{ color: CYAN }}>ruido</span>
              <br />
              Diseñado por IA
            </h1>
            ...
            <p className="text-base sm:text-lg leading-relaxed max-w-md" style={{ color: MUTED }}>
              Diagnosticamos y transformamos cualquier espacio con 20 años de
              experiencia acústica y modelos de IA entrenados con cientos de
              proyectos reales.
            </p>
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href="/asesor"
                className="px-6 py-3 rounded-xl text-sm font-semibold tracking-wide transition-opacity hover:opacity-85"
                style={{ backgroundColor: CYAN, color: BG }}
              >
                Analizar mi espacio
              </Link>
          ...
          {/* Right — dark card with room + stats */}
          <div
          ...
          >
            ...
            >
              ...
              <span className="ml-3 text-[11px] font-medium" style={{ color: MUTED }}>
                acustega — análisis.acústico
              </span>
            </div>

            {/* Isometric room */}
            <div className="w-full aspect-[3/2] bg-[#090e16] p-4">
              <IsometricRoom />
            </div>
            ...
            >
              {STATS.map(({ value, label }, i) => (
                ...
                >
                  ...
                  <span className="text-[10px] font-medium tracking-wider uppercase" style={{ color: MUTED }}>
                    {label}
                  </span>
                </div>
              ))}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold leading-tight">
              Elige tu{" "}
              <span style={{ color: AMBER }}>plan</span>
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {PRICING.map((plan) => (
              <div
                ...
                style={{
                  backgroundColor: plan.highlight ? SURFACE2 : SURFACE,
                  border: `1px solid ${plan.highlight ? CYAN + "60" : BORDER}`,
                  boxShadow: plan.highlight ? `0 0 40px ${CYAN}14` : "none",
                }}
              >
                {/* Popular badge */}
                {plan.highlight && (
                  <span
                    className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-[10px] font-bold tracking-[0.15em] uppercase"
                    style={{ backgroundColor: CYAN, color: BG }}
                  >
                    Popular
                  </span>
                ...
                <div className="flex flex-col gap-1">
                  <span
                    ...
                    style={{ color: plan.highlight ? CYAN : MUTED }}
                  >
                    {plan.name}
                  </span>
                ...
                <div className="flex items-end gap-1">
                  <span className="text-4xl font-bold" style={{ color: CREAM }}>
                    {plan.price}
                  </span>
                ...
                <ul className="flex flex-col gap-2.5 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-[13px]" style={{ color: MUTED }}>
                      <Check color={plan.highlight ? CYAN : AMBER} />
                      {f}
                    </li>
                  ))}
                </ul>
                ...
                <Link
                  href="/asesor"
                  className="text-center py-2.5 rounded-xl text-sm font-semibold tracking-wide transition-opacity hover:opacity-85"
                  style={
                    plan.highlight
                      ? { backgroundColor: CYAN, color: BG }
                      : { border: `1px solid ${BORDER}`, color: CREAM, backgroundColor: "transparent" }
                  }
                >
                  {plan.cta}
                </Link>
            ...
            {SOCIALS.map(({ label, href, icon }) => (
              <a
                ...
                onMouseEnter={(e) => {
                  ...
                  (e.currentTarget as HTMLElement).style.borderColor = `${CYAN}50`;
                }}
                onMouseLeave={(e) => {
      ...
      <a
        ...
        className="fixed bottom-6 right-6 z-50 w-13 h-13 rounded-full flex items-center justify-center transition-transform hover:scale-110 active:scale-95"
        style={{
          width: 52,
          height: 52,
          backgroundColor: WA_GREEN,
          boxShadow: `0 4px 20px ${WA_GREEN}55`,
        }}
      >
